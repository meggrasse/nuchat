...
var MESSAGE = "Message";
var findConvIdRegex = new RegExp(/^.*['"](.*?)['"].*$/);
...
var Query = (function (_Root) {
  function Query() {
    ...
    _get(Object.getPrototypeOf(Query.prototype), "constructor", this).call(this, options);
    this.data = [];
  ...
  _createClass(Query, {
    ...
    _run: {
      ...
      value: function _run() {
        ...
        var pageSize = Math.min(this.paginationWindow - this._sentPaginationWindow, MAX_PAGE_SIZE);
        ...
        } else if (pageSize === 0) {} else if (this.model === CONVERSATION) {
    ...
    _runConversation: {
      ...
      value: function _runConversation(pageSize) {
        ...
        var fromId = lastConversation && !lastConversation.id.match(/temp_/) ? "&from_id=" + lastConversation.id : "";
        ...
        this.client.xhr({
          url: firingRequest,
          method: "GET",
          sync: false }, function (results) {
          return _this._processRunResults(results, firingRequest);
        });
    ...
    _runMessage: {
      ...
      value: function _runMessage(pageSize) {
        ...
        var fromId = lastMessage && !lastMessage.id.match(/temp_/) ? "&from_id=" + lastMessage.id : "";
        ...
        if (this.predicate.match(findConvIdRegex)) {
          ...
          if (uuid) {
            (function () {
              if (!_this._predicate) _this._predicate = conversationId;
              var conversation = _this.client.getConversation(conversationId);
              if (conversation && conversation.lastMessage && lastMessage && lastMessage.id === conversation.lastMessage.id) fromId = "";
              var newRequest = "conversations/" + uuid + "/messages?page_size=" + pageSize + "" + fromId;

              // Don't query on temporary ids, nor repeat still firing queries
              if (!_this._predicate.match(/temp_/) && newRequest !== _this._firingRequest) {
                _this.isFiring = true;
                _this._firingRequest = newRequest;
                _this._allResultsLoaded = false;
                _this.client.xhr({
                  url: newRequest,
                  method: "GET",
                  sync: false }, function (results) {
                  return _this._processRunResults(results, newRequest);
                });
                _this._sentPaginationWindow += pageSize;
              }
              if (_this.data.length === 0) {
                if (conversation && conversation.lastMessage) {
                  _this.data = [_this._getData(conversation.lastMessage)];
                  // Trigger the change event
                  _this.trigger("change", {
                    type: "data",
                    data: _this._getData(conversation.lastMessage),
                    query: _this,
                    target: _this.client });
                }
              }
    ...
    _processRunResults: {
      ...
      value: function _processRunResults(results, requestUrl) {
        if (requestUrl !== this._firingRequest) {
          return;
        }this.isFiring = false;
        this._firingRequest = "";
        if (results.success) {
          ...
          if (this._sentPaginationWindow !== this.paginationWindow) {
            var totalSize = results.xhr.getResponseHeader("Layer-Count");
            var totalSizeNumb = Number(totalSize);
    ...
    _appendResults: {
      ...
      value: function _appendResults(results) {
        ...
        newResults.forEach(function (itemIn) {
          var index = undefined;
          var item = _this.client._getObject(itemIn.id);
          if (_this.model === MESSAGE) {
            index = _this._getInsertMessageIndex(item, data);
          } else {
            index = _this._getInsertConversationIndex(item, data);
          }
          data.splice(index, 0, _this._getData(item));
        });
        ...
        this.trigger("change", {
          ...
          data: results.data.map(function (item) {
            return _this._getData(_this.client._getObject(item.id));
          }),
          query: this,
          target: this.client });
      }
          ...
          case "messages":
            if (this.model === MESSAGE) {
              ...
              for (var index = 0; index < this.data.length; index++) {
                ...
                if (conversation.lastMessage && conversation.lastMessage.id === id) {
    ...
    _handleConversationChangeEvent: {
    ...
    _handleConversationAddEvent: {
      value: function _handleConversationAddEvent(evt) {
        ...
        var list = evt.conversations.filter(function (conversation) {
          return _this._getIndex(conversation.id) === -1;
        });
        ...
        if (list.length) {
          (function () {
            ...
            list.forEach(function (conversation) {
              var newIndex = _this._getInsertConversationIndex(conversation, data);
              data.splice(newIndex, 0, _this._getData(conversation));
            });
            ...
            list.forEach(function (conversation) {
              var item = _this._getData(conversation);
              _this.trigger("change", {
                type: "insert",
                index: _this.data.indexOf(item),
                target: item,
                query: _this });
            });
          })();
    ...
    _handleConversationRemoveEvent: {
      value: function _handleConversationRemoveEvent(evt) {
        var _this = this;

        evt.conversations.forEach(function (conversation) {
          var index = _this._getIndex(conversation.id);
          if (index !== -1) {
            if (_this.dataType === "object") {
              _this.data = [].concat(_toConsumableArray(_this.data.slice(0, index)), _toConsumableArray(_this.data.slice(index + 1)));
            } else {
            ...
            _this.trigger("change", {
              type: "remove",
              index: index,
              target: _this._getData(conversation),
              query: _this });
          }
        });
      }
    ...
    _handleMessageConvIdChangeEvent: {
    ...
    _handleMessageAddEvent: {
      value: function _handleMessageAddEvent(evt) {
        ...
        var list = evt.messages.filter(function (message) {
          return message.conversationId === _this._predicate;
        }).filter(function (message) {
          return _this._getIndex(message.id) === -1;
        }).map(function (message) {
          return _this._getData(message);
        });

        // Add them to our result set and trigger an event for each one
        if (list.length) {
          (function () {
            var data = _this.data = _this.dataType === "object" ? [].concat(_this.data) : _this.data;
            list.forEach(function (item) {
              var index = _this._getInsertMessageIndex(item, data);
